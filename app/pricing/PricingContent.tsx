...
type PackageKey = 'start' | 'grow' | 'scale';

const PACKAGE_TITLES: Record<PackageKey, string> = {
  start: 'Пакет START',
  grow: 'Пакет GROW',
  scale: 'Пакет SCALE',
};

function getDefaultMessage(key: PackageKey): string {
  switch (key) {
    case 'start':
      return 'Расскажите, что хотите запустить в ближайшие 1–2 месяца: какой у вас проект, на какой платформе работаете и какие задачи по автоматизации хотите закрыть в первую очередь.';
    case 'grow':
      return 'Опишите, где вы сейчас упираетесь в ручной труд: заявки, выдача доступов, вебинары, отчёты, внутренние процессы. На какой платформе работаете и какая команда.';
    case 'scale':
      return 'Расскажите, до каких объёмов уже доросли (по запускам, лидам, выручке) и что хочется масштабировать. Какие процессы уже автоматизированы, а какие пока живут в ручном режиме.';
  }
}
...
export default function PricingContent() {
  ...
  const openPackage = (key: PackageKey) => setActivePackage(key);
  ...
  return (
    <>
          ...
          <section className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div id="start" className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                ...
                <a
                  href="/contact?plan=Пакет%20START%20(базовая%20автоматизация)"
              ...
              <div id="grow" className="flex flex-col rounded-2xl border border-emerald-500 bg-slate-900/80 p-5 shadow-[0_0_0_1px_rgba(16,185,129,0.35)]">
                ...
                <a
                  href="/contact?plan=Пакет%20GROW%20(рост%20и%20систематизация)"
                  className="mt-4 inline-flex items-center justify-center rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-emerald-400 transition"
                >
                  Заказать пакет GROW
                </a>
              ...
              <div id="scale" className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                ...
                <a
                  href="/contact?plan=Пакет%20SCALE%20(масштабирование,%20сети)"
                  className="mt-4 inline-flex items-center justify-center rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-emerald-400 transition"
                >
      ...
      {/* ===== МОДАЛЬНОЕ ОКНО ДЛЯ ЗАЯВКИ ПО ПАКЕТУ ===== */}
      {activePackage && (
        <div
        ...
        >
          ...
          >
            ...
            <ContactForm
              compact
              source="pricing"
              context={`package:${activePackage}`}
              defaultMessage={getDefaultMessage(activePackage)}
            />
          </div>
        </div>
      )}

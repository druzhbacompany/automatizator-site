  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-50">
      <div className="max-w-5xl mx-auto px-4 pt-6 pb-10">
        ...
        {hasIntegrations && (
  <CollapsibleSection title="Интеграции и стек" initialOpen>
    <p className="max-w-2xl text-xs leading-relaxed text-slate-300 sm:text-sm">
      Используемые сервисы и технологии. При необходимости можем адаптировать
      стек под ваши ограничения и уже выбранные платформы.
    </p>
    <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-200 sm:text-sm">
      {service.integrations?.map((item) => (
        ...
        {hasCases && (
  <CollapsibleSection title="Примеры кейсов" initialOpen>
    <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
      {service.cases?.map((c) => (
        ...
        >
          ...
          {c.result && (
            <p className="mt-2 text-sm text-emerald-300">{c.result}</p>
          )}
          {c.note && (
        ...
        {hasRisks && (
  <CollapsibleSection title="Риски и ограничения">
    <p className="max-w-2xl text-xs leading-relaxed text-slate-300 sm:text-sm mb-3">
      Честно проговариваем, от чего зависит результат и что может пойти не по
      плану — ещё до старта проекта, а не по факту.
    </p>
    <ul className="space-y-2.5 text-xs text-slate-200 sm:text-sm">
      {service.risks?.map((item) => (
        ...
        {hasFaq && (
  <CollapsibleSection title="Частые вопросы по этой услуге">
